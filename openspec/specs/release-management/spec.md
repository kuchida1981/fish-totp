...
`workflow_dispatch` トリガーによる手動リリース用 GitHub Actions ワークフロー（`.github/workflows/release.yml`）の挙動を定義する。バージョン入力のバリデーション、main ブランチ限定の実行、リリース前のテスト実行、`TOTP_VERSION` の反映とタグ作成の順序、GitHub Release の自動生成を含む。
