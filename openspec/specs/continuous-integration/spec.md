...
GitHub Actions による fish-totp プラグインの自動検証。push/pull_request をトリガーとしたワークフローの起動、CI 環境への依存パッケージ（fish/jq/oathtool/python3/fisher）のセットアップ、`fisher install .` 検証とコマンドテストスイートの実行・失敗時のジョブ失敗化を含む。
